...
var HTML_ify = function(val, raw_page) {
  ...
  if (_.isArray(val)) {
    ...
    if (err)
      return {error: err, raw: HTML_ify.compile(compiled)};

    return {html: HTML_ify.compile(compiled), js: ""};
  };
...
HTML_ify.Page.prototype.last = function () {
  var this_page = this;
  var last = _.last(this_page.origin);
  if (!last)
    return null;
  return HTML_ify.Page.Element.new( cheerio.load(last).root().children().last() );
};
