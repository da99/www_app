...
Ok.escape_attrs = function (attrs, e) {
  var sanitized = {};
  var err = null;

  _.find(attrs, function (v, k) {
    var pair = Ok.escape_attr(k, v, e);
    if (IS_ERROR(pair)) {
      err = pair;
      return pair;
    }
...
Ok.escape_attr = function (k, v, e) {
  if (_.contains(['id', 'name', 'type'], k) && !v.match(VALID_HTML_ID))
    return new Error("Invalid chars in " + e.tag + " " + k + ": " + v);

  if (!k.match(VALID_HTML_ID))
    return new Error("Invalid chars in " + e.tag + " attribute name: " + k);
...
DSL.form = function (attrs, childs) {
  if (!_.isArray(childs))
  ...
  var id     = (attrs.length === 1) ? DSL.generate_id('form') : attrs[0];
  var action = _.last(attrs);
  var form   = New_Element('form', {
    id      : id,
    action  : action,
    ok_type : 'form'
  });
...
Ok.Element.new = function (raw_tag, raw_attrs, raw_content, page) {
  ...
  e.attrs = Ok.escape_attrs(raw_attrs, e);
