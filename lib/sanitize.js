...
E.uri = function (raw, name) {
  ...
  if (parse.errors.length)
    return new Error(name + ": address is invalid: " + parse.errors[0]);
...
E.num_of_lines = function (raw_val) {
  if (!_.isNumber(raw_val) || _.isNaN(raw_val))
    return new Error("Must be a number: " + JSON.stringify(raw_val));

  if (raw_val < 1 || raw_val > 250)
    return new Error("Number out of range: " + JSON.stringify(raw_val));

  return raw_val;
};

E.string_in_array = function (unk) {
  if (_.isArray(unk) && unk.lenght === 1 && _.isString(unk[0]))
    return unk;
  return new Error("Must be a string within an array: " + JSON.stringify(unk));
};
