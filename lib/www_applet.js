...
WWW_Applet.Computers = {
  ...
  "IS": function (sender, to, args) {
    if (arguments.length === 2) { // run as native function
      var orig_name = arguments[0];
      var name  = standard_key(orig_name);
      var value = arguments[1];

      if (this.values.hasOwnProperty(name)) {
        throw new Error("Value already created: " + inspect(orig_name));
      }
      this.values[name] = last(value);

      return value;
    }

    return this.IS(last(sender.stack), last(args));
  },

  "IS A COMPUTER" : function (sender, to, tokens) {
    var name = standard_key(last(sender.stack));
    if (!this.computers[name]) {
      this.computers[name] = [];
    }
    this.computers[name].push(tokens);
