...
, Escape      = require('www_applet/lib/escape').Escape
;
...
function run_content(meta, content) {
  NOT_READY();
}
...
function num_of_lines() {
  NOT_READY();
}

function string(val) {
  if (!_.isString(val))
    return {error: new Error('Not a string: ' + JSON.stringify(val))};
  return {update: Escape(val)};
}
...
function href(val) {
  return find_err(val, string, function (val) {
    return {update: Escape.uri(val)};
  });
}
