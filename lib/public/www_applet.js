  ...
  var funcs =  {
    ...
    "and": function (scope, raw_args) {
      ...
      if (!_.isBoolean(first)) {
        throw new Error('Right hand value is not a bool: ' + inspect(first));
      }
      ...
      if (first === true) {
        ...
        if (!_.isBoolean(second)) {
          throw new Error('Left hand value is not a bool: ' + inspect(second));
        }
    ...
    "if true": function (scope, raw_args) {
      var first = _.last(scope.stack);
