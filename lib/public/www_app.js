...
(function () {
  ...
  var Scope = function (parent) {
    ...
    var on = function (name, raw_target) {
      if (!_.contains(['mousedown', 'mouseup', 'focus', 'blur', 'click'], name)) {
      ...
      target.on(name, function (e) {
        ...
        run_does(name, e.target, e);
        submit_form(name, e.target, e);
    ...
    var submit_form = function (event_name, raw_target, e) {
      if (event_name !== 'click')
        return false;
    ...
    var run_does = function (event_name, raw_target, e) {

      log(event_name, events)
      var target = $(raw_target);
      ...
      _.each(events, function (meta) {
        ...
        if (first === event_name) {
          ...
          if (second && !third) {
            c = child();
            relative = relative_id(target, second);
            if (!relative) { return false; }
            c.push(relative);
          } else {
            if (!$(second).is(target)) { return false; }

            c = child();
            if (third) {
              c.push(relative_id(target, third));
            } else {
              c.push(id(second));
            }
          ...
          if (target.hasClass(first)) {
            ...
            child().push(relative).run(meta.code);
          }
  ...
  WWW_App = {
    ...
    run   : function (code_array) {
      var scope = (new Scope());
      // ========= Setup default actions for buttons: ===============
      _.each( $('button'), function (raw_b) {
        scope.on('click', raw_b);
      }); // === _.each

      return scope.run(code_array);
    }
  };
